...
def file_generator(input_path, batch_size):
    ...
    user_id,hist_len, hist_movie_id, pos_movie_id, neg_movie_id = init_output()
    ...
    while True:
        ...
        with open(input_path, 'r') as f:
            for line in f.readlines():
                ...
                pos_movie_id.append(np.array([int(i) for i in buf[3].strip().split(",")]))
                neg_movie_id.append(np.array([int(i) for i in buf[4].strip().split(",")]))
                ...
                if cnt % batch_size == 0 or cnt == num_lines:
                    ...
                    yield [user_id, hist_movie_id,hist_len, pos_movie_id, neg_movie_id], label
                    
                    # user_id, gender, age, occupation, zip, hist_movie_id, hist_len, pos_movie_id, neg_movie_id = init_output()
                    user_id, hist_movie_id,hist_len, pos_movie_id, neg_movie_id = init_output()
